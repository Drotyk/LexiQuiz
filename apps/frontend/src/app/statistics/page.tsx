  ...
  if (isAuthLoading || isLoading) {
    return (
      <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center">
        <div className="text-gray-400 font-mono animate-pulse">Loading statistics...</div>
      </div>
    );
  }
  ...
  return (
    <main className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6">
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 shadow-xl space-y-1">
          <span className="text-xs text-gray-400 font-medium">New Words</span>
          <div className="text-3xl font-black text-gray-400">{overview?.newWords || 0}</div>
        </div>

        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 shadow-xl space-y-1">
          <span className="text-xs text-gray-400 font-medium">In Learning</span>
          <div className="text-3xl font-black text-gray-400">{overview?.learningWords || 0}</div>
        </div>

        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 shadow-xl space-y-1">
          <span className="text-xs text-gray-400 font-medium">In Reviewing</span>
          <div className="text-3xl font-black text-gray-400">{overview?.reviewingWords || 0}</div>
        </div>

        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 shadow-xl space-y-1">
          <span className="text-xs text-gray-400 font-medium">Mastered Words</span>
          <div className="text-3xl font-black text-gray-400">{overview?.masteredWords || 0}</div>
        </div>
      </div>

      {/* Quiz & Speed Performance */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 shadow-xl flex items-center space-x-4">
          ...
          <div>
            ...
            <div className="text-xs text-gray-400">7-Day Quiz Accuracy</div>
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 shadow-xl flex items-center space-x-4">
          ...
          <div>
            ...
            <div className="text-xs text-gray-400">Avg Answer Speed</div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 sm:p-8 shadow-xl space-y-6">
        ...
        {difficultWords.length === 0 ? (
          <div className="p-8 text-center bg-gray-950/40 border border-gray-800/80 rounded-xl text-gray-400 text-sm">
            ...
            <table className="w-full text-left text-sm text-gray-300">
              ...
              <tbody className="divide-y divide-gray-800/60 font-mono text-xs">
                {difficultWords.map((word) => (
                  <tr key={word.wordId} className="hover:bg-gray-800/40">
                    <td className="p-4 font-bold text-white font-sans text-sm">{word.term}</td>
                    <td className="p-4 text-gray-300 font-sans text-sm">{word.translation}</td>
                    <td className="p-4 text-gray-400 font-sans">{word.setName}</td>
                    <td className="p-4 text-center text-gray-400 font-bold">{word.incorrectAnswers}</td>
                    <td className="p-4 text-right text-gray-200">{word.accuracyPercent}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 sm:p-8 shadow-xl space-y-6">
        ...
        <div className="grid grid-cols-7 gap-2 sm:gap-4 text-center">
          {activity.map((item, i) => (
            <div key={i} className="p-3 rounded-xl bg-gray-950 border border-gray-800 space-y-1">
              <div className="text-[10px] font-mono text-gray-500">{item.date.slice(5)}</div>
              <div className="text-lg font-bold text-white">{item.count}</div>
              <div className="text-[10px] text-gray-400">{item.correct} correct</div>
            </div>
          ))}
        </div>
      </div>
